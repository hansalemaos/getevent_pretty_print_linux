...
static int print_possible_events(int fd, int print_flags, tabsmsub &submap)
{
    uint8_t *bits = NULL;
    ssize_t bits_size = 0;
    const char *label;
                        ...
                        tabsmsub &submap)
{
    ...
    if (type == 3 && code == 0)
    {
        printf("%04x %04x %08x\t%d\t%d\t%d\t%d\t%d", type, code, value, type, code, value,
               screen_width * value / submap[3][0], -1);
    }
    else if (type == 3 && code == 1)
    {
        printf("%04x %04x %08x\t%d\t%d\t%d\t%d\t%d", type, code, value, type, code, value, -1,
               screen_height * value / submap[3][1]);
    }
    else
    {
        printf("%04x %04x %08x\t%d\t%d\t%d\t%d\t%d", type, code, value, type, code, value, -1, -1);
    }
}
...
static int open_device(const char *device, int print_flags)
{
    ...
    {
        absmap[nfds];
        print_possible_events(fd, print_flags, absmap[nfds]);
    }
...
int main(int argc, char *argv[])
{
    if (argc != 3)
    {
        printf("Usage: geteventall screen_width screen_height\ne.g. geteventall 1920 1080\n");
        exit(1);
    }
        ...
        for (i = 1; i < nfds; i++)
        {
            if (ufds[i].revents)
            {
                if (ufds[i].revents & POLLIN)
                {
                    ...
                    print_event(event.type, event.code, event.value, print_flags, screen_width, screen_height,
                                absmap[i]);
